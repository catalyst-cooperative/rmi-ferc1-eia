...
def prep_master_parts_eia(plant_parts_df, deprish, key_ppe):
    ...
    # RESTRICT_MATCH_COLS
    options_index = (
        deprish[RESTRICT_MATCH_COLS]
        .drop_duplicates()
        .set_index(RESTRICT_MATCH_COLS)
        .index
    )
    plant_parts_df = (
        plant_parts_df.reset_index()  # convert the record_id_eia index to a column
        .set_index(RESTRICT_MATCH_COLS)
        .loc[options_index]
        .reset_index()
        .convert_dtypes(convert_floating=False)
...
def get_plant_year_util_list(plant_name, deprish, ppe, key_ppe):
    ...
    options_index = (
        deprish.loc[deprish.plant_part_name == plant_name, RESTRICT_MATCH_COLS]
        .drop_duplicates()
        .set_index(RESTRICT_MATCH_COLS)
        .index
    )
    ...
    names = ppe.set_index(RESTRICT_MATCH_COLS).loc[options_index, key_ppe].to_list()
    return names
...
def get_fuzzy_matches(deprish, ppe, key_deprish, key_ppe, threshold=75):
    ...
    match_tuple_series = deprish[key_deprish].apply(
        lambda x: process.extractOne(
            x,
            get_plant_year_util_list(x, deprish, ppe, key_ppe),
...
def add_overrides(deprish_match, file_path_deprish, sheet_name_output):
    ...
    try:
        ...
        deprish_match_full = (
            ...
            .drop_duplicates()
            .assign(
                record_id_eia=lambda x: x.record_id_eia_override.fillna(
                    x.record_id_eia_fuzzy
                )
            )
        )
...
def match_deprish_eia(deprish, plant_parts_eia, sheet_name_output):
    ...
    deprish_match = (
        deprish_match.drop(
            columns=[
                c
                for c in deprish_match
                if c in ppe_non_id_cols and c not in RESTRICT_MATCH_COLS
            ]
        )
        .merge(
    ...
    deprish_match = deprish_match.loc[
        :, first_cols + [x for x in deprish_match.columns if x not in first_cols]
    ]
...
def add_record_id_fuzzy(deprish, plant_parts_eia, key_ppe):
    ...
    right_on = ["report_year", "utility_id_pudl", key_ppe, "plant_id_eia"]
...
def execute(
    ...
    deprish_match = match_deprish_eia(
        deprish, plant_parts_eia, sheet_name_output=sheet_name_output
    )
